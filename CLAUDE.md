...
- **Name**: kitchen-provisioner-chef-ice
- **Type**: Ruby gem — Test Kitchen provisioner plugin
- **Purpose**: Thin provisioner that inherits from `ChefInfra` (kitchen-omnibus-chef) and overrides only the install path for chef-ice (chef-client 19+).
...
- Default `product_name` is `chef-ice`.
...
```
kitchen-provisioner-chef-ice.gemspec
Gemfile
Makefile
LICENSE
CHANGELOG.md
